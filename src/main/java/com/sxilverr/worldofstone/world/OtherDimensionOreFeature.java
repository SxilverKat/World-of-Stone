...
public class OtherDimensionOreFeature extends Feature<NoneFeatureConfiguration> {
        ...
        if (genObsidian) {
            for (int i = 0; i < 16; i++) {
                ...
                if (level.getBlockState(pos).is(Blocks.OBSIDIAN)) {
                    ...
                    RegistrySupplier<Block> oreBlock = WosBlocks.ORES.get("obsidian_" + ore.suffix);
    ...
    private boolean placeVein(WorldGenLevel level, BlockPos center, int size, OreVariant ore, boolean isNether, boolean isEnd, RandomSource rand) {
        ...
        for (BlockPos p : candidates) {
            ...
            RegistrySupplier<Block> oreBlock = WosBlocks.ORES.get(hostName + "_" + ore.suffix);
    ...
    public static int attemptsForOreNether(OreVariant ore) {
        ...
        return Math.max(0, (int) Math.round(base * com.sxilverr.worldofstone.config.WosConfig.netherOreAttemptsMultiplier));
    }
    ...
    public static int attemptsForOreEnd(OreVariant ore) {
        ...
        return Math.max(0, (int) Math.round(base * com.sxilverr.worldofstone.config.WosConfig.endOreAttemptsMultiplier));
    }
