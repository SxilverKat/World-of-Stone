...
public final class WosFossilDropHandler {
    ...
    private static ItemStack pickRandomFossil() {
        ...
        if (roll < 3) return new ItemStack(WosItems.FOSSILS.get(com.sxilverr.worldofstone.api.enums.FossilVariant.BONE_FOSSIL).get());
        if (roll < 5) return new ItemStack(WosItems.FOSSILS.get(com.sxilverr.worldofstone.api.enums.FossilVariant.RIB_FOSSIL).get());
        if (roll < 7) return new ItemStack(WosItems.FOSSILS.get(com.sxilverr.worldofstone.api.enums.FossilVariant.SHELL_FOSSIL).get());
        if (roll < 8) return new ItemStack(WosItems.FOSSILS.get(com.sxilverr.worldofstone.api.enums.FossilVariant.AMMONITE_FOSSIL).get());
        return new ItemStack(WosItems.FOSSILS.get(com.sxilverr.worldofstone.api.enums.FossilVariant.SKULL_FOSSIL).get());
    }
}
